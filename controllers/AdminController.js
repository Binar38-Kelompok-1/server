...
class AdminController {
  ...
  static async editProfilePost(req, res) {
    try {
      ...
      const usernameDupe = await db("petugas")
        .whereNot({ id: req.session.aidi })
        .where({ username: username.replace(/\s+/g, "").toLowerCase() });
      const noTelpDupe = await db("petugas")
        .whereNot({ id: req.session.aidi })
        .where({ no_telp: no_telp.replace(/\s+/g, "").substring(1) });
      ...
      if (
        no_telp.replace(/\s+/g, "").length < 9 ||
        no_telp.replace(/\s+/g, "").length > 12
      ) {
  ...
  static async masyarakatDetail(req, res) {
    try {
      const data = await db("masyarakat").where({
        id: req.params.idMasyarakat,
      });
      const nama = await db("petugas")
        .where({ id: req.session.aidi })
        .select("nama");
  ...
  static async masyarakatEditPost(req, res) {
    try {
      ...
      const nikDupe = await db("masyarakat")
        .whereNot({ id: req.params.idMasyarakat })
        .where({ nik: nik.replace(/\s+/g, "") });
      const noTelpDupe = await db("masyarakat")
        .whereNot({ id: req.params.idMasyarakat })
        .where({ no_telp: no_telp.replace(/\s+/g, "").substring(1) });
  ...
  static async petugasRegisPost(req, res) {
    try {
      ...
      const data = await db("petugas")
        .where({ id: req.session.aidi })
        .select("nama");
      const usernameDupe = await db("petugas").where({
        username: username.replace(/\s+/g, "").toLowerCase(),
      });
      const noTelpDupe = await db("petugas").where({
        no_telp: no_telp.replace(/\s+/g, "").substring(1),
      });
  ...
  static async sudahBalasDetail(req, res) {
    try {
      ...
      res.render("admin/selesai-detail", {
        layout: "layout/admin-sidebar",
  ...
  static async riwayat(req, res) {
    try {
      const data = await db("balasan")
        ...
        .where({ id_petugas: req.session.aidi });
      const nama = await db("petugas")
        .where({ id: req.session.aidi })
        .select("nama");
